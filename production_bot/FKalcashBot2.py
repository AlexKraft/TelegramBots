...
#data = 'db.json'
#API_TOKEN = '989604812:AAE1NWU3CwhDfo41ucg80nE2aboimTmlDtQ'   # TEST
...
def loadDB():
    with open(data, 'r') as f:  # , encoding='utf-8'
        db = json.load(f)
    return db


def uploadDB(db):
    with open(data, 'w+') as f:  # , encoding='utf-8'
...
'''
    сюда мы попадаем после того как поделились на команды и админам вывелись
    кнопочки
...
'''
    ...
    reg     - проверяет есть ли игра и если да вносит ID в 
            db["game"]["players"] и сохраняет адрес сообщения что б потом его 
            редактировать
...
'''
    ...
    add     - добавляет имя в db["game"]["added"] и db["game"]["list"],
            ...
            и редактирует сообщение у всех кто подписался на игру
            db["game"]["players"]
